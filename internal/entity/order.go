...
type Order struct {
	...
	Delivery Delivery `json:"delivery" gorm:"foreignKey:OrderUID;references:OrderUID;constraint:OnDelete:CASCADE"`
	Payment  Payment  `json:"payment"  gorm:"foreignKey:OrderUID;references:OrderUID;constraint:OnDelete:CASCADE"`
	Items    []Item   `json:"items"    gorm:"foreignKey:OrderUID;references:OrderUID;constraint:OnDelete:CASCADE"`
}

// Delivery represents delivery details for an order
type Delivery struct {
	OrderUID string `json:"-" gorm:"type:varchar(255);primaryKey"`
...
type Payment struct {
	...
	OrderUID string `json:"-" gorm:"type:varchar(255);not null;index"`
...
type Item struct {
	OrderUID string `json:"-" validate:"required" gorm:"type:varchar(255);primaryKey;autoIncrement:false;uniqueIndex:idx_order_rid"`
	Rid      string `json:"rid"    validate:"required,alphanumunicode,min=5,max=64" gorm:"type:varchar(255);primaryKey;autoIncrement:false;uniqueIndex:idx_order_rid"`
